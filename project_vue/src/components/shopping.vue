     		  ...
     		  <el-tabs v-model="activeName" @tab-click="handleClick" class="tab">
			    <el-tab-pane label="产品详情" name="first" class="a">
			    	...
			    	<div class="detail1">
			    		<p @click="math">推荐产品</p>
						<ul class="oss">
							<li v-for="(item,index) in arr" @click="add(item,index)">
								<img :src="item.img"/>
								<p>￥{{item.ppp}}</p>
							</li>
						</ul>
...
<script>
	export default{
		...
		methods:{
	     ...
	     math(){
	     	let te=JSON.parse(localStorage.getItem('data2'))
	     	this.arr=te
	     	console.log(te)
	     ...
	     add(item,index){
	     	this.index2=index
	     	console.log(index)
	     	localStorage.setItem('data04',JSON.stringify(this.arr))
	     	localStorage.setItem('data05',JSON.stringify(this.index2))
	     	this.$router.push({
	     		path:"/shopping1"
	     	}) 
	     },
...
//			localStorage.setItem('data03',JSON.stringify(this.list))
	       ...
	       HiddenClick () {
			...
			 },
	     sub(){
	      	  this.$alert('<div class="inp"><p>您的姓名</p><input type="text" placeholder="请输入您的姓名" /><p>您的电话</p><input type="text" placeholder="请输入您的电话" /><p>备注</p><input type="text" placeholder="" id="inp"/><p @click="yan">验证码</p><input type="text" placeholder="请输入验证码" id="ipn1"/><span id="span">1314</span><button id="tijiao">提交</button></div>', '提交表单', {
...
//	         console.log($(".el-message-box__wrapper p .inp #span").html())
	       })
		   
	     },
		},
		mounted(){
			let temp1=JSON.parse(localStorage.getItem("data2"))
			let temp=JSON.parse(localStorage.getItem('data1'))
			this.info=temp1
			this.index=temp
			console.log(temp1)
			console.log(temp)
